...
4. 需求阶段主要工作：
    ...
    - （1）需求获取：深入实际，在充分理解用户需求的基础上，获取系统的综合需求。
    - （2）需求转化与分类：将收集到的需求进行整理，并转化为需求分析记录表中的内容，同时明确需求的基本属性。
    - （3）需求分析与提炼：对已获取的需求进行深入分析和提炼，进行各种应用场景场景分析。
    - （4）需求建模：对分析和提炼后的需求进行抽象描述，建立目标系统的逻辑模型。
    - （5）需求规约:对需求模型进行精确的、形式化的描述，为计算机系统的实现提供基础。
    - （6)需求验证与评审:发现需求分析的错误和缺陷，进行需求评审会议，邀请相关利益方参与，对需求进行评审和确认。
    -  (7)需求跟踪与管理:持续跟踪和管理需求的变化，注意用户的需求转变。
5. 非功能需求：
    - 性能需求：
