...
4. 需求阶段主要工作：
    ...
    - 3.竞品分析：
    - （1）确定竞品：找到类似软件，如Google相册、百度识图.
    - （2)功能对比：列出功能，特别是数据库、相册权限、文字搜索等
    -  (3)优缺点分析：分析用户体验、搜索速度、准确度等.
    -  (4)用户反馈：查看用户对这些竞品的评价.
    -  (5)数据对比：了解用户量、活跃度等市场情况.
      
    - ii)需求分析：展示如何分析需求，确定项目的关键功能和非功能需求。
    - （1）需求获取：通过访谈、问卷调查、竞品分析等方式，全面收集用户需求，确保收集到的需求信息准确、完整，并尽可能避免遗漏或误解。
    - （2）需求分类与分析：将收集到的需求进行整理，使用KANO模型等工具对需求进行分类，明确哪些是必要型需求等，考虑时间、地点等因素，确保需求理解的准确性。
    - （3）需求建模：进行抽象描述，使用数据流图表示系统中数据的流动和处理过程。
    - （4）需求规约:进行精确的、形式化的描述，如编写需求规格说明书，详细阐述系统的功能需求和非功能需求。
    - （5)需求验证与评审:邀请相关利益方参与，对需求进行评审和确认，根据需求规格说明书制定详细的测试计划，确保系统实现与需求一致。
    -  (6)需求跟踪与管理:持续跟踪和管理需求的变化，可建立需求跟踪矩阵，对需求变更进行审批、记录和传播。
5. 非功能需求：
    - 性能需求：
