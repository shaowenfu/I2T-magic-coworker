...
4. 需求阶段主要工作：
    - i）需求收集：说明如何收集用户需求，包括用户访谈、问卷调查等。
    - 1.用户访谈：
    - （1）.选取不同年龄层、职业背景及手机使用习惯的用户作为访谈对象，确保样本多样性。
    - （2）访谈内容设计：准备一份详细的访谈大纲，涵盖用户对图文管理、搜索功能的需求与期望。
    - （3）实施访谈：线上视频或线下面对面交流，记录用户反馈，观察用户操作习惯，捕捉潜在需求。
    - （4）归纳整理：提炼用户需求和建议，形成初步需求清单。
    - 2.问卷调查：
    - （1）问卷设计：设计包含用户对图文助手软件的基本需求、功能偏好、界面设计、性能期望等。
    - （2）分发渠道：通过社交媒体、专业论坛、用户社群等渠道分发问卷以覆盖目标用户群体。
    - （3)数据分析：运用统计软件进行数据分析，识别用户需求的共性与差异。
    - (4)反馈循环：根据问卷分析结果，调整问卷内容或进行补充访谈。
      
    - ii)需求分析：展示如何分析需求，确定项目的关键功能和非功能需求。
    - （1）需求获取：此阶段的目标是深入实际，在充分理解用户需求的基础上，获取系统的综合需求。
    - （2）需求转化与分类：将收集到的需求进行整理，并转化为需求分析记录表中的内容，同时明确需求的基本属性，这包括需求转化和需求分类两大方面。
    - （3）需求分析与提炼：对已获取的需求进行深入分析和提炼，进行各种应用场景场景分析，确保需求理解的准确性。以及流程分析，分析满足需求的关键路径，如何正确地搜索到想要的内容，确保需求的完整性和可行性。价值分析，评估需求的价值，包括用户需求价值和产品方案价值。
    - （4）需求建模：对分析和提炼后的需求进行抽象描述，建立目标系统的逻辑模型。主要含有绘制数据流图表示系统中数据的流动和处理过程。 再者使用业务活动清单列出这个软件系统在达成每个图文搜索目的时需要执行的所有业务活动。
    - （5）需求规约:对需求模型进行精确的、形式化的描述，为计算机系统的实现提供基础。编写需求规格说明书：详细阐述图文搜索系统的功能需求、性能需求、安全需求等非功能需求,记录想要达成的效果和功能。
    - （6)需求验证与评审:发现需求分析的错误和缺陷，需求说明准确、完整地表达系统的主要特性。如进行需求评审会议，邀请相关利益方参与，对需求进行评审和确认。
    -  (7)需求跟踪与管理:持续跟踪和管理需求的变化，注意用户的需求转变。例如建立需求跟踪矩阵，记录需求与系统设计、实现、测试等阶段之间的对应关系。
5. 非功能需求：
    - 性能需求：
...
6. 期望的效果：
    ...
    - （1）功能实现：文字搜索照片功能：用户输入关键词后，软件能够迅速在数据库中搜索并展示与关键词相关的照片。照片搜索文字功能：用户上传或选择照片后，软件能够准确识别照片中的文字，并在数据库中搜索相关的内容，展示给用户。
    - （2）搜索速度：确保软件的搜索速度足够快，用户无需等待过长时间即可看到搜索结果，提高OCR技术的准确性，确保照片中的文字能够被正确识别。
    - （3）界面美观：软件的界面设计简洁、美观，符合用户的审美需求。
    - （4）数据安全：确保用户的照片和文字数据在存储和传输过程中得到充分的保护，防止数据泄露或被恶意利用。
    - (5) 用户反馈：收集用户的反馈和建议，不断优化和改进软件的功能和用户体验。
    - （6）交互流畅：软件的交互流程顺畅，用户可以轻松完成搜索操作。
    - （7）多模态检索功能实现：用户可以通过文字搜索照片，也可以通过照片搜索相关的文字信息。检索结果应该准确、全面，并包含不同模态的相关信息。
7. 团队分工：
